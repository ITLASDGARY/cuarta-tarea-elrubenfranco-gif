...
namespace EjerciciosCS
{
    ...
    public class Ejercicios
    {
        ...
        public List<string> CrearListaCompras()
        {
            ...
            while (true)
            {
                Console.WriteLine("Ingrese un producto (escriba 'salir' para terminar):");
        ...
        public bool BuscarNumeroDinamico(int numeroBuscado)
        {
            ...
            _arrayGenerado = new int[10];

            for (int i = 0; i < _arrayGenerado.Length; i++)
            {
                _arrayGenerado[i] = random.Next(0, 101);
            }

            Console.WriteLine("Números generados:");
            for (int i = 0; i < _arrayGenerado.Length; i++)
            {
                Console.Write(_arrayGenerado[i] + " ");
            }
            Console.WriteLine();

            for (int i = 0; i < _arrayGenerado.Length; i++)
            {
                if (_arrayGenerado[i] == numeroBuscado)
                {
                    return true;
                }
            }
